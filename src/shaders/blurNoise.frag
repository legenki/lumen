...
// Sources
uniform sampler2D u_src; // source image (RGBA)
uniform sampler2D u_noise; // blue noise (tileable)
uniform sampler2D u_mask; // mask, alpha = effect strength

// Parameters
uniform vec2 u_resolution; // frame size in pixels
uniform float u_radius; // maximum blur radius in pixels
uniform float u_noiseIndependence; // 0.05..0.3
uniform float u_noiseScale; // 0.5..4.0
uniform int u_samples; // 1..MAX_SAMPLES
uniform bool u_maskUse; // whether to use the mask
uniform float u_mix; // 0..1 - pass opacity
uniform int u_blendMode; // Photoshop-style blending mode

// Constants
const int MAX_SAMPLES = 24;
const vec2 NOISE_JITTER = vec2(1.079173, 0.878642);

// ---------- Blend Modes Utilities ----------
float luma(vec3 c) {
...
// Blur with premultiplied alpha
vec4 stochasticBlur(vec2 uv, float radiusPx) {
  if (radiusPx <= 0.0 || u_samples <= 0) {
    return texture(u_src, uv); // PM sample
  }

  vec2 texelSize = 1.0 / u_resolution;
  vec2 radiusUV = texelSize * radiusPx;

  vec4 acc = vec4(0.0); // PM RGBA sum
  float count = 0.0;

  for (int i = 0; i < MAX_SAMPLES; ++i) {
    if (i >= u_samples) break;

    vec2 noiseUV = (uv + 0.5) * u_noiseScale + NOISE_JITTER * float(i) * u_noiseIndependence;
    noiseUV = fract(noiseUV);

    vec3 n = texture(u_noise, noiseUV).rgb;

    vec2 offset01 = n.rg;
    vec2 offset = (offset01 - 0.5) * 2.0 * radiusUV;

    vec2 baseUV = uv + offset;
    vec4 s0 = texture(u_src, baseUV); // PM-sample
...
void main() {
  ...
  vec2 uv = vTexCoord;
  vec4 Bpm = texture(u_src, uv);
  float Ab = Bpm.a;

  if (u_radius <= 0.0 || u_samples <= 0) {
    fragColor = Bpm;
    return;
  }

  // Mask
  float maskFactor = u_maskUse ? clamp(texture(u_mask, uv).a, 0.0, 1.0) : 1.0;

  float localRadius = u_radius * maskFactor;
  float k = clamp(u_mix, 0.0, 1.0); // global mix

  if (localRadius <= 0.0 || k <= 0.0) {
    fragColor = Bpm;
    return;
  }

  // Blur (PM)
  vec4 blurred = stochasticBlur(uv, localRadius);
  float As = blurred.a;
  if (As <= EPS) {
    fragColor = Bpm;
    return;
  ...
  vec3 Cb = Ab > EPS ? Bpm.rgb / Ab : vec3(0.0); // Backdrop STRAIGHT
  vec3 Cs = As > EPS ? blurred.rgb / As : vec3(0.0); // Blur STRAIGHT
  ...
  vec3 PM_out = mix(Bpm.rgb, PM_blur, k);
