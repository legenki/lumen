...
void main() {
  ...
  // Source layer params (color + alpha from UI + mask)
  float As = clamp(u_color.a * mask, 0.0, 1.0);
  vec3 Cs = u_color.rgb;

  if (u_alphaMode == 0) {
    // --- MODE 0: Ignore Source Alpha ---

    // Photoshop-style blending on straight colors
    vec3 Bcs = applyBlend(u_blendMode, Cb, Cs);

    // Compose in PREMULT (W3C/PS)
    ...
    // --- MODE 1: "Fade by Source Alpha" —
    float k = As;
    vec3 blended = applyBlend(u_blendMode, Cb, Cs); // straight
    vec3 Cout = mix(Cb, blended, k);

    // Alpha is derived from the source
    fragColor = vec4(Cout * Ab, Ab); // PREMULT
  }
}
