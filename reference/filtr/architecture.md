...
> **Мост к спеке Lumen.** Термины спеки (`docs/superpowers/specs/2026-07-10-lumen-migration-design.md`)
> соотносятся так: *Persisted* ≈ старый `appState` (cnv/rec) + сериализуемая часть стека;
> *Runtime* ≈ старые `runtimeState` + `runtime` (FBO, шрифты, курсор кадра). Старый `uiState`
> (зеркало для Tweakpane) в новой архитектуре аналога не имеет — panelBuilder биндится к
> state напрямую.
...
(Имя поля в старом коде — с двумя подчёркиваниями; спека Lumen называет новое поле
`maskMembers` — это осознанное переименование, не опечатка.)
